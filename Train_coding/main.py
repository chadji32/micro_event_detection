...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default=None, help="Path to save the model.")
    parser.add_argument("--log_file", type=str, default="model/logs_no_aug.txt", help="Path to save the logs.")
    parser.add_argument("--prediction_file", type=str, default="model/predictions_no_aug.csv", help="Path to save the predictions.")
...
def sketch_effect(image):
    ...
    final_image = Image.fromarray(inverted_edges)
...
def custom_transform(image):
    ...
    image = sketch_effect(image)
    
    return image
    
# Function for augmentation techniques
def get_transforms(train=True):
    transforms_list = [T.Resize((m_config.image_size, m_config.image_size))]
    
    if train and m_config.augmentation:
        if "flip" in m_config.augmentation:
            transforms_list.append(T.RandomHorizontalFlip())
        if "color_jitter" in m_config.augmentation:
            transforms_list.append(T.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2))
        if "rotation" in m_config.augmentation:
            transforms_list.append(T.RandomRotation(degrees=30))
        if "sketch" in m_config.augmentation:
            transforms_list.append(T.Lambda(lambda img: custom_transform(img)))
...
def transform(frames, train=True):
    resize_transform = get_transforms(train)
...
def main():
    ...
    dri_001 = os.path.join(parent_directory, 'DRI-001_frames')
    dri_006 = os.path.join(parent_directory, 'DRI-006_frames')
    patient_dirs = []
    for path in os.listdir(dri_001):
        patient_dirs.append(os.path.join(dri_001, path))
    
    for path in os.listdir(dri_006):
        patient_dirs.append(os.path.join(dri_006, path))
    
    random.shuffle(patient_dirs)

    train_size = int(0.85 * len(patient_dirs))
    val_size = len(patient_dirs) - train_size
    train_dataset, val_dataset = random_split(patient_dirs, [train_size, val_size])

    train_dataset = FrameDataset(patient_dirs=train_dataset, transform=lambda frames: transform(frames, train=True), num_frames=m_config.num_frames)
    val_dataset = FrameDataset(patient_dirs=val_dataset, transform=lambda frames: transform(frames, train=False), num_frames=m_config.num_frames)
    
    train_loader = DataLoader(train_dataset, batch_size=m_config.batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=m_config.batch_size, shuffle=False)
    
    # Load timeSformer model and it's configurations
    config = TimesformerConfig.from_pretrained("facebook/timesformer-base-finetuned-k400")
    config.image_size = m_config.image_size  # Set image size to 128
    ...
    num_epochs = m_config.num_epochs
    for epoch in range(num_epochs):
        print(f"Epoch {epoch+1}/{num_epochs}")
        train(model, train_loader, optimizer, device, epoch, num_epochs, m_config.log_file, m_config.model)
        validate(model, val_loader, criterion, device, m_config.log_file)
